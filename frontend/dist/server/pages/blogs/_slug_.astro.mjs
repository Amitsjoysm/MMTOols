...
import { s as ssrBlogsApi } from '../../chunks/ssr-api_ip_KgNt5.mjs';
...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (rawJsonLd && typeof rawJsonLd === "string") {
    try {
      rawJsonLd = JSON.parse(rawJsonLd);
    } catch {
    }
  }
  const existingFaqs = (() => {
  ...
  const articleJsonLd = {
    ...
    "@graph": [
      ...
      ...existingFaqs.length > 0 ? [{
        "@type": "FAQPage",
        "mainEntity": existingFaqs.slice(0, 10).map((q) => ({
          "@type": "Question",
  ...
  const processedContent = isHtmlContent ? blog.content : blog.content.split("\n").map((line) => {
