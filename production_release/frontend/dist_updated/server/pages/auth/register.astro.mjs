import { c as createComponent, b as addAttribute, e as renderHead, a as renderTemplate, d as renderScript } from '../../chunks/astro/server_CZk8NOd-.mjs';
import 'piccolore';
import 'clsx';
/* empty css                                        */
...
const $$Register = createComponent(async ($$result, $$props, $$slots) => {
  ...
  try {
    const res = await fetch(`${"http://localhost:8001"}/api/site-settings/logo`);
    if (res.ok) {
      const d = await res.json();
      logoUrl = d.logo_url;
      siteName = d.site_name || "MarketMindAI";
    }
  } catch {
  }
  return renderTemplate`<html lang="en" class="motion-safe:scroll-smooth 2xl:text-[20px]"> <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${metadata.title}</title><meta name="description"${addAttribute(metadata.description, "content")}><meta name="robots" content="noindex, nofollow">${renderHead()}</head> <body class="antialiased bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 min-h-screen flex items-center justify-center"> <div class="w-full max-w-md px-4 py-8"> <!-- Logo and Header --> <div class="text-center mb-8"> <a href="/" class="inline-block mb-4"> <div class="flex items-center justify-center"> ${logoUrl ? renderTemplate`<img${addAttribute(logoUrl, "src")}${addAttribute(siteName, "alt")} class="h-14 w-auto">` : renderTemplate`<div class="flex items-center gap-2"> <svg class="w-12 h-12 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path> </svg> <span class="text-2xl font-bold text-gray-900 dark:text-white">${siteName}</span> </div>`} </div> </a> <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Create Account</h1> <p class="text-gray-600 dark:text-gray-400">Join ${siteName} today</p> </div> <!-- Register Form --> <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8"> <form id="register-form" class="space-y-6"> <!-- Full Name --> <div> <label for="full_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
Full Name
