...
@router.get("/api/site-settings/analytics")
async def get_analytics_settings(db: Session = Depends(get_db)):
    """Get Analytics and AdSense settings (public endpoint for SSR)"""
    ...
    return {
        ...
        "adsense_id": _get('adsense_id'),
        "custom_head_code": _get('custom_head_code'),
        "gtm_id": _get('gtm_id'),
    }

@router.post("/api/superadmin/site-settings/analytics")
async def update_analytics_settings(
    data: dict,
    current_superadmin: User = Depends(get_current_superadmin),
    db: Session = Depends(get_db)
):
    """Update Analytics and AdSense settings (SuperAdmin only)"""
    allowed_keys = ['analytics_id', 'adsense_id', 'custom_head_code', 'gtm_id',
                    'site_name', 'contact_email', 'support_email', 'twitter_handle',
    ...
    for key, value in data.items():
        ...
        if setting:
            ...
            setting = SiteSettings(key=key, value=str(value) if value is not None else '', description=f'{key} setting')
            db.add(setting)
        updated[key] = value
    db.commit()
